...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const checkLoginStatus = async () => {
      ...
      console.table(checkToken?.status);

      if (
        checkToken?.permissions.data?.permission !== "admin" ||
        !token ||
            ...
            <MenubarMenu>
              ...
              <MenubarContent>
                <MenubarItem>
                  <Link
                    to="/dashboard/"
                    className="text-white flex items-center text-[15px] gap-3 transition-all hover:text-green-500"
                  >
